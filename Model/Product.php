...
class Product
{
    ...
    public function allSorted(string $sortBy = 'id', string $sortOrder = 'DESC'): array
    {
        $allowedSorts = ['id', 'name', 'price', 'calories', 'is_approved', 'added_by'];
        $sortBy = in_array($sortBy, $allowedSorts) ? $sortBy : 'id';
        $sortOrder = strtoupper($sortOrder) === 'ASC' ? 'ASC' : 'DESC';
        
        $query = "SELECT * FROM produit ORDER BY {$sortBy} {$sortOrder}";
    ...
    public function approvedSorted(string $sortBy = 'id', string $sortOrder = 'DESC'): array
    {
        $allowedSorts = ['id', 'name', 'price', 'calories', 'is_approved', 'added_by'];
        $sortBy = in_array($sortBy, $allowedSorts) ? $sortBy : 'id';
        $sortOrder = strtoupper($sortOrder) === 'ASC' ? 'ASC' : 'DESC';
        
        $query = "SELECT * FROM produit WHERE is_approved = 1 ORDER BY {$sortBy} {$sortOrder}";
        $result = $this->connection->query($query);
    ...
    public function pendingSorted(string $sortBy = 'id', string $sortOrder = 'DESC'): array
    {
        $allowedSorts = ['id', 'name', 'price', 'calories', 'is_approved', 'added_by'];
        $sortBy = in_array($sortBy, $allowedSorts) ? $sortBy : 'id';
        $sortOrder = strtoupper($sortOrder) === 'ASC' ? 'ASC' : 'DESC';

        $query = "SELECT * FROM produit WHERE is_approved = 0 ORDER BY {$sortBy} {$sortOrder}";
        $result = $this->connection->query($query);
