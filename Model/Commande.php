...
class Commande
{
    ...
    public function allSorted(string $sortBy = 'id', string $sortOrder = 'DESC', ?string $search = null): array
    {
        $allowedSorts = ['id', 'buyer_name', 'buyer_phone', 'buyer_email', 'buyer_address', 'quantity', 'total_price', 'product_id'];
        ...
        if ($search !== null && $search !== '') {
            $sql .= ' WHERE c.buyer_name LIKE ? OR c.buyer_phone LIKE ? OR c.buyer_email LIKE ?';
            $types .= 's';
            $params[] = '%' . $search . '%';
            $types .= 's';
            $params[] = '%' . $search . '%';
    ...
    public function update(int $id, array $data): bool
    {
        $statement = $this->connection->prepare(
            'UPDATE commande
             SET product_id = ?, buyer_name = ?, buyer_phone = ?, buyer_address = ?, buyer_email = ?, quantity = ?, total_price = ?
             WHERE id = ?'
        );
