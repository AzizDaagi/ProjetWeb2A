...
<div class="animate-fade-in">
    ...
    <div class="glass-card" style="margin-bottom: 3rem; background: linear-gradient(145deg, rgba(30, 41, 59, 0.8), rgba(15, 23, 42, 0.9));">
        ...
        <div style="display: flex; gap: 1.5rem;">
            <div style="background: rgba(255,255,255,0.05); padding: 1rem; border-radius: 8px; flex: 1; text-align: center;">
                <h4 style="color: var(--text-muted); margin-bottom: 0.5rem;">Dur√©e Totale</h4>
                <div style="font-size: 1.5rem; font-weight: bold;">‚è± <?= $activite['duree_minutes'] ?> min</div>
            </div>
            <div style="background: rgba(255,255,255,0.05); padding: 1rem; border-radius: 8px; flex: 1; text-align: center;">
                <h4 style="color: var(--text-muted); margin-bottom: 0.5rem;">Calories Br√ªl√©es</h4>
    ...
    <div>
        <h3 style="margin-bottom: 1.5rem; border-bottom: 1px solid var(--card-border); padding-bottom: 0.5rem;">D√©tail des Exercices</h3>
        
        <?php if(empty($exercices)): ?>
            <p style="color: var(--text-muted); font-style: italic;">Aucun exercice renseign√© par l'administrateur.</p>
        <?php else: ?>
                    ...
                    <div class="glass-card exercice-item" data-muscle="<?= htmlspecialchars($ex['muscle_principal']) ?>" style="display: flex; justify-content: space-between; align-items: stretch; padding: 1rem 1.5rem;">
                        <div>
                            <h4 style="font-size: 1.2rem; margin-bottom: 0.5rem; color: #fff;">
                                ...
                                <span style="font-size: 0.8rem; background: <?= $ex['niveau_difficulte'] == 'Avanc√©' ? 'var(--error)' : ($ex['niveau_difficulte'] == 'Interm√©diaire' ? '#fbbf24' : '#34d399') ?>; color: #000; padding: 2px 6px; border-radius: 4px; margin-left: 8px;">
                            ...
                            <div style="color: var(--primary); font-weight: 600;">
                                <?= $ex['series'] ?> s√©ries √? <?= $ex['repetitions'] ?> r√©p. | <span style="color: #fbbf24;">??• <?= $ex['calories_estimees'] ?> kcal</span>
                            </div>
                        </div>
                    </div>
                <?php endforeach; ?>
