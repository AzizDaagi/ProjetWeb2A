...
<section class="products-shell admin-dashboard-shell">
    ...
    <form method="GET" action="<?= htmlspecialchars(route_url('order.list')) ?>" class="list-toolbar" style="margin: 18px 0 24px; display: flex; gap: 12px; flex-wrap: wrap; align-items: end;">
        <div class="field" style="min-width: 220px; flex: 1;">
            <label for="q">Search</label>
            <input id="q" name="q" type="text" value="<?= htmlspecialchars((string) ($query ?? '')) ?>" placeholder="Buyer name or phone">
        </div>
        ...
        <a href="<?= htmlspecialchars(route_url('order.list')) ?>" class="btn section-action btn-secondary-link">Reset</a>
    </form>
