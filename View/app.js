...
function initVoiceControl() {
    ...
    var routeMap = [
        ...
        { action: 'admin_dashboard', aliases: ['dashboard', 'tableau de bord', 'admin dashboard'] },
        { action: 'admin_requests', aliases: ['demandes nutrition', 'requete nutrition', 'gestion nutrition'] },
        ...
        { action: 'activites', aliases: ['activite sportif', 'activite sportive', 'sport'] },
    ...
    function buildActionUrl(action) {
        return getAppBasePath() + '/index.php?action=' + encodeURIComponent(action);
    }
...
function initAdminUsersList() {
    ...
    function createActionCell(user) {
        ...
        editLink.href = getAppBasePath() + '/index.php?action=edit-user&id=' + encodeURIComponent(normalizeValue(user.id));
        editLink.className = 'btn-edit';
        editLink.innerHTML = '<i class="fa-solid fa-pen"></i> Modifier';

        var form = document.createElement('form');
        form.method = 'POST';
        form.action = getAppBasePath() + '/index.php?action=delete-user';
    ...
    if (exportButton) {
        exportButton.addEventListener('click', function () {
            var url = getAppBasePath() + '/index.php?action=users-report&search=' + encodeURIComponent(searchInput.value || '');
...
function getAppBasePath() {
    ...
    var indexPosition = path.lastIndexOf('/index.php');

    if (indexPosition !== -1) {
        return path.slice(0, indexPosition) || '';
    }
...
function initFullScreenHandControl() {
    if (!document.querySelector('[data-enable-gesture]')) {
        return;
    }
