<div class="container admin-dashboard admin-form-page product-form-shell">
    <h1><i class="fa-solid fa-box-open icon"></i> Ajouter un produit</h1>
    <p class="subtitle">Creation directe depuis le back office.</p>
    <?php if (!empty($error ?? '')): ?>
    ...
    <form method="POST" enctype="multipart/form-data" class="product-form" novalidate>
        ...
        <div class="field">
            ...
            <input id="added_by" name="added_by" value="<?= htmlspecialchars((string) ($old['added_by'] ?? ($_SESSION['user_name'] ?? 'Admin'))) ?>" placeholder="Vendeur" required>
        </div>
        ...
        <button type="submit"><i class="fa-solid fa-floppy-disk icon success"></i> Ajouter le produit</button>
    </form>
</div>
