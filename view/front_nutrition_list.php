...
<div class="animate-fade-in" style="max-width: 900px; margin: 0 auto; padding: 2rem;">
    <div class="glass-card" style="padding: 2rem; margin-bottom: 2rem;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; flex-wrap: wrap; gap: 1rem;">
            <h2 style="color: var(--primary); margin: 0;"><i class="fa-solid fa-clipboard-list"></i> Historique Demandes</h2>
            ...
            <a href="index.php?action=export_nutrition_pdf&search=<?= urlencode($_GET['search'] ?? '') ?>&sort=<?= urlencode($_GET['sort'] ?? 'date_desc') ?><?= $embedQuery ?>" 
        ...
        <div style="display: grid; gap: 1.5rem;">
            <?php foreach($requests as $req): ?>
                <div class="glass-card" style="display: flex; justify-content: space-between; align-items: center; padding: 1.5rem;">
                    <div>
                        <h4 style="color: var(--primary); margin-bottom: 0.8rem;"><i class="fa-solid fa-bullseye"></i> <?= htmlspecialchars($req['current_goal']) ?></h4>
                        <div style="display: flex; gap: 1rem; margin-bottom: 0.8rem; font-size: 0.85rem;">
                            <span style="background: rgba(255,255,255,0.1); padding: 4px 8px; border-radius: 4px; color: var(--text-main);"><i class="fa-solid fa-weight-scale"></i> <?= $req['current_weight'] ?> kg</span>
                            <span style="background: rgba(255,255,255,0.1); padding: 4px 8px; border-radius: 4px; color: var(--text-muted);"><i class="fa-solid fa-calendar-days"></i> <?= date('d/m/Y', strtotime($req['created_at'])) ?></span>
                        </div>
                        <p style="color: var(--text-main); margin-top: 5px; font-style: italic; opacity: 0.8;">"<?= htmlspecialchars($req['message'] ?: 'Aucun detail') ?>"</p>
                    ...
                    <div style="display: flex; gap: 1rem;">
                        <a href="index.php?action=edit_nutrition_request&id=<?= $req['id'] ?><?= $embedQuery ?>" class="btn btn-outline" style="border-color: #f59e0b; color: #f59e0b;">Modifier</a>
                        <a href="index.php?action=delete_nutrition_request&id=<?= $req['id'] ?><?= $embedQuery ?>" class="btn" style="background: var(--error);" onclick="return true;">Supprimer</a>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
    <?php elseif(isset($_GET['email'])): ?>
        <p style="text-align: center; color: var(--text-muted);"><i class="fa-solid fa-magnifying-glass"></i> Aucune demande trouvee.</p>
    <?php endif; ?>
    </div>

    <div style="text-align: center; margin-top: 3rem;">
        <a href="index.php?action=nutrition_request<?= $embedQuery ?>" class="btn btn-outline">&larr; Nouvelle Demande</a>
    </div>
</div>
