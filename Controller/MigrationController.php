...
class MigrationController extends BaseController
{
    ...
    /**
     * Add buyer_email column to commande table
     */
    public function migrateAddBuyerEmail()
    {
        $conn = Database::connection();

        // Check if column already exists
        $result = $conn->query("SHOW COLUMNS FROM commande LIKE 'buyer_email'");
        if ($result && $result->num_rows > 0) {
            $this->render('migration/success', [
                'message' => "✅ Column buyer_email already exists on commande",
                'columns' => []
            ]);
            return;
        }

        $sql = "ALTER TABLE commande ADD COLUMN buyer_email VARCHAR(255) DEFAULT NULL";

        if ($conn->query($sql) === TRUE) {
            $message = "✅ Migration successful: added buyer_email to commande";
